...
const bestMovies = document.querySelector(".directors__best-list");

directors.forEach((director, index) => {
  const item = document.createElement("li");
  item.classList.add("directors__item");

  const name = document.createElement("span");
  name.classList.add("directors__name");
  name.textContent = `${index + 1}. ${director.name}`;

  const career = document.createElement("div");
  career.classList.add("directors__career");
  career.textContent = director.career;

  const filmography = document.createElement("a");
  filmography.classList.add("directors__filmography");
  filmography.href = director.films;
  filmography.textContent = "Фильмография";
  filmography.target = "_blank";

  item.appendChild(name);
  item.appendChild(career);
  item.appendChild(filmography);
